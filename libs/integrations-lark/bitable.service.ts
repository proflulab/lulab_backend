  ...
  /**
    ...
    async batchGetRecords(
        ...
        const request: BatchGetRecordRequest = {
            ...
            ...(options?.userIdType && { user_id_type: options.userIdType }),
            ...(options?.withSharedUrl !== undefined && { with_shared_url: options.withSharedUrl }),
            ...(options?.automaticFields !== undefined && { automatic_fields: options.automaticFields }),
        };
    ...
    async upsertRecord(
        ...
        const { matchFields, matchMode = 'exact', caseSensitive = false } = queryConditions;
        const { mergeFields = false, excludeFields = [], returnFullRecord = true } = options || {};

        try {
            // 1. 构建查询条件
            const searchFilter = this.buildSearchFilter(fields, matchFields, matchMode, caseSensitive);
        ...
        for (let i = 0; i < records.length; i += batchSize) {
            ...
            const batchPromises = batch.map(async (record, batchIndex) => {
                ...
                try {
                    ...
                    this.logger.error(`批量upsert中第${i + batchIndex}条记录失败: ${error.message}`);
                    throw error;
                }
            });
