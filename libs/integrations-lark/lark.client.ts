...
export class LarkClient {
  ...
  async createBitableRecord(
    ...
    try {
      this.logger.debug(
        `Creating Bitable record in app: ${request.app_token}, table: ${request.table_id}`,
      );
  ...
  async updateBitableRecord(
    ...
    try {
      this.logger.debug(
        `Updating Bitable record ${request.record_id} in app: ${request.app_token}, table: ${request.table_id}`,
      );
  ...
  async searchBitableRecords(
    ...
    try {
      this.logger.debug(
        `Searching Bitable records in app: ${request.app_token}, table: ${request.table_id}`,
      );
   ...
   */
  async *searchBitableRecordsIterator(
    options: SearchRecordIteratorOptions,
  ): AsyncGenerator<LarkRecord, void, unknown> {
    try {
      this.logger.debug(
        `Searching Bitable records with iterator in app: ${options.app_token}, table: ${options.table_id}`,
      );

      const iteratorPromise =
        this.client.bitable.v1.appTableRecord.searchWithIterator({
          path: {
  ...
  async listBitableRecords(
    ...
    try {
      this.logger.debug(
        `Listing Bitable records in app: ${request.app_token}, table: ${request.table_id}`,
      );
   ...
   */
  async *listBitableRecordsIterator(
    request: ListRecordRequest,
  ): AsyncGenerator<LarkRecord, void, unknown> {
    try {
      this.logger.debug(
        `Listing Bitable records with iterator in app: ${request.app_token}, table: ${request.table_id}`,
      );

      const iteratorPromise =
        this.client.bitable.v1.appTableRecord.listWithIterator({
          path: {
            app_token: request.app_token,
            table_id: request.table_id,
          },
  ...
  async deleteBitableRecord(
    ...
    try {
      this.logger.debug(
        `Deleting Bitable record ${request.record_id} in app: ${request.app_token}, table: ${request.table_id}`,
      );
  ...
  async batchCreateBitableRecords(
    ...
    try {
      this.logger.debug(
        `Batch creating Bitable records in app: ${request.app_token}, table: ${request.table_id}`,
      );
   ...
   */
  async batchUpdateBitableRecords(
    request: BatchUpdateRecordRequest,
  ): Promise<BatchUpdateRecordResponse> {
    try {
      this.logger.debug(
        `Batch updating Bitable records in app: ${request.app_token}, table: ${request.table_id}`,
      );
   ...
   */
  async batchGetBitableRecords(
    request: BatchGetRecordRequest,
  ): Promise<BatchGetRecordResponse> {
    try {
      this.logger.debug(
        `Batch getting Bitable records in app: ${request.app_token}, table: ${request.table_id}`,
      );
      ...
      const response = await this.client.bitable.v1.appTableRecord.batchGet({
        ...
        data: {
          ...
          ...(request.user_id_type && { user_id_type: request.user_id_type }),
          ...(request.with_shared_url !== undefined && {
  ...
  async batchDeleteBitableRecords(
    ...
    try {
      this.logger.debug(
        `Batch deleting Bitable records in app: ${request.app_token}, table: ${request.table_id}`,
      );
