...
export async function createUsers(prisma: PrismaClient): Promise<CreatedUsers> {
  ...
  // 并行创建普通用户
  const normalUserPromises = NORMAL_USER_PROFILES.map(async (profile, index) => {
    ...
    return createUserWithProfile(
      ...
      userPasswordHash,
      {
        ...profile,
        country: COUNTRY,
        dateOfBirth: new Date(1990 + userNumber, userNumber % 12, (userNumber * 5) % 28 + 1),
      }
    )
  })
  ...
  await Promise.all([
    ...
    ...normalUsers.map(user => assignUserRole(prisma, user.id, roles.user.id)),
  ])
