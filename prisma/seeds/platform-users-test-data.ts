 ...
 * 生成平台用户ID
 */
function generatePlatformUserId(
...
async function createTestPlatformUser(
  ...
  // 根据平台选择姓名类型
  const name = config.useChineseName
    ? generateChineseName()
    : generateEnglishName();
  ...
  try {
    ...
    const platformUser = await prisma.platformUser.create({
      data: {
        ...
        phoneHash:
          platform === $Enums.MeetingPlatform.TENCENT_MEETING
    ...
    console.log(
      `✅ 创建平台用户 ${index}: ${config.name} - ${name.fullName} (${email})${userLink}`,
    );
  } catch (error: any) {
    if (error.code === 'P2002') {
  ...
  try {
    ...
    for (let i = 0; i < count; i += batchSize) {
      ...
      for (let j = i; j < Math.min(i + batchSize, count); j++) {
        ...
        // 先明确 batch 类型为 Promise<void>[] 或 Promise<any>[]
        const batch: Promise<void>[] = []; // 或者 Promise<any>[]

        batch.push(
          createTestPlatformUser(selectedPlatform, j + 1, linkToLocalUser),
        );
      }
...
// 导出函数供其他模块使用
export {
  createTestPlatformUsers,
  cleanTestPlatformUsers,
  showTestPlatformUsersStats,
};
