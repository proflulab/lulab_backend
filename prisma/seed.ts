 ...
 */
function topologicalSort(tables: string[], dependencies: TableDependencies): string[] {
 ...
 */
async function dropAllTables(options: DatabaseOperationOptions = {}): Promise<void> {
 ...
 * 确认删除操作
 */
async function confirmDropOperation(tables: string[], force: boolean): Promise<void> {
  console.log('\n⚠️  ⚠️  ⚠️  警告 ⚠️  ⚠️  ⚠️')
...
async function seedDatabase(): Promise<void> {
  ...
  try {
    // 步骤 1: 创建基础数据结构
    const { userData, permissionData, organizationData, channelData, projectData, curriculumData, productData } = 
...
async function createBasicData() {
  ...
  return { userData, permissionData, organizationData, channelData, projectData, curriculumData, productData }
}
 ...
 * 执行数据库操作
 */
async function executeDatabaseOperation(command: string, force: boolean): Promise<void> {
  switch (command) {
    case 'clean':
      await cleanDatabase()
      break
