...
describe('BitableService (Integration Tests)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!appToken || !tableId) {
      throw new Error(
        '测试配置不完整，请在.env.test中配置 LARK_TEST_APP_TOKEN 和 LARK_TEST_TABLE_ID',
      );
    }
  });
  ...
  describe('基础CRUD操作', () => {
    ...
    it('应该能够更新记录', async () => {
      ...
      const updateResult = await service.updateRecord(
        appToken,
        tableId,
        recordId!,
        updateFields,
      ...
      expect(updateResult.data?.record?.fields?.['测试文本']).toBe(
        '更新后的值',
      );
    ...
    it('应该能够删除记录', async () => {
      ...
      // 删除记录
      const deleteResult = await service.deleteRecord(
        appToken,
        tableId,
        recordId!,
      );
  ...
  describe('批量操作', () => {
    ...
    it('应该能够批量更新记录', async () => {
      ...
      const createResult = await service.batchCreateRecords(
        appToken,
        tableId,
        createRecords,
      );
      ...
      const updateRecords = [
        {
          ...
          fields: { 测试文本: '批量更新后1', 测试数字: 100 },
        },
        {
          record_id: recordIds[1],
          fields: { 测试文本: '批量更新后2', 测试数字: 200 },
        },
      ];

      const updateResult = await service.batchUpdateRecords(
        appToken,
        tableId,
        updateRecords,
      );
      ...
      expect(updateResult.data?.records?.[0]?.fields?.['测试文本']).toBe(
        '批量更新后1',
      );
    });
    ...
    it('应该能够批量获取记录', async () => {
      ...
      // 批量获取记录
      const getResult = await service.batchGetRecords(
        appToken,
        tableId,
        recordIds,
      );
    ...
    it('应该能够批量删除记录', async () => {
      ...
      // 批量删除记录
      const deleteResult = await service.batchDeleteRecords(
        appToken,
        tableId,
        recordIds,
      );
  ...
  describe('Upsert操作', () => {
    it('应该能够执行upsert操作 - 创建新记录', async () => {
      ...
      const result = await service.upsertRecord(appToken, tableId, testFields, {
        matchFields: ['唯一标识'],
        matchMode: 'exact',
      });
    ...
    it('应该能够执行upsert操作 - 更新现有记录', async () => {
      ...
      expect(result.record.fields?.['唯一标识']).toBe(uniqueId);
      expect(Number(result.record.fields?.['测试数字'])).toBe(
        originalNum + 1000,
      );
    });
    ...
    it('应该能够执行批量upsert操作', async () => {
      ...
      expect(
        results.every((r) => ['created', 'updated'].includes(r.action)),
      ).toBe(true);
    ...
    it('应该能够执行批量upsert操作 - 部分更新部分创建', async () => {
      ...
      const updateResult = results.find(
        (r) => r.record.fields?.['唯一标识'] === uniqueId1,
      );
      const createResultFound = results.find(
        (r) => r.record.fields?.['唯一标识'] === uniqueId2,
      );

      expect(updateResult?.action).toBe('updated');
      expect(createResultFound?.action).toBe('created');

      expect(updateResult?.record.fields?.['测试文本']).toBe(
        '更新已存在的记录',
      );
  ...
  describe('迭代器功能', () => {
    ...
    it('应该能够使用搜索迭代器', async () => {
      ...
      // 使用批量获取验证记录
      const getResult = await service.batchGetRecords(appToken, tableId, [
        recordId!,
      ]);
      expect(getResult.data).toBeDefined();
  ...
  describe('错误处理', () => {
    ...
    it('应该处理无效的表格ID', async () => {
      const result = await service.createRecord(appToken, 'invalid-table-id', {
        测试文本: 'test',
      });
    ...
    it('应该处理无效的字段名', async () => {
      const result = await service.createRecord(appToken, tableId, {
        不存在的字段: 'test',
      });
      expect(result.code).not.toBe(0);
    });
