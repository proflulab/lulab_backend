 ...
 * @LastEditTime: 2025-12-24 05:00:43
...
export class MeetingStartedHandler extends BaseEventHandler {
  ...
  constructor(
    private readonly meetingRecordService: MeetingBitableService,
  ...
  async handle(payload: TencentEventPayload, index: number): Promise<void> {
    ...
    const userRecordPromises = [
      this.meetingRecordService.upsertMeetingUserRecord(operator),
    ];

    // 如果操作者和创建者不是同一人，添加创建者记录处理
    if (operator.uuid !== meeting_info.creator.uuid) {
      void this.meetingRecordService.upsertMeetingUserRecord(
