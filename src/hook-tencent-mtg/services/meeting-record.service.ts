...
import { TencentEventOperator, TencentEventMeetingInfo } from '../types';
...
export class MeetingRecordService {
  ...
  async updateMeetingParticipants(
    ...
    try {
      ...
      if (operator) {
        const userRecord = await this.meetingUserService.findMeetingUserByUuid(
          operator.uuid,
        );
        ...
        if (!operator_id) {
          operator_id =
            await this.meetingUserService.upsertMeetingUserRecord(operator);
        }
      }

      // 查找创建者记录ID
      const creatorRecord = await this.meetingUserService.findMeetingUserByUuid(
      ...
      if (!creator_record_id) {
        creator_record_id =
          await this.meetingUserService.upsertMeetingUserRecord(
            meetingInfo.creator,
          );
      }
