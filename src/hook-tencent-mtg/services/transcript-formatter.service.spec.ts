...
describe('TranscriptFormatterService', () => {
  ...
  describe('formatTranscript', () => {
    it('should return empty string when transcript is null or undefined', () => {
      expect(service.formatTranscript(undefined)).toBe('');
    });
    ...
    it('should format transcript with single paragraph and single sentence', () => {
      const transcript: RecordingTranscriptResponse = {
        minutes: {
          paragraphs: [
            {
              pid: 'p1',
      ...
      expect(result).toBe('张三(01:00:00)：大家好，今天我们讨论项目进展');
    });
    ...
    it('should format transcript with single paragraph and multiple sentences', () => {
      ...
      const result = service.formatTranscript(transcript);
      expect(result).toBe('李四(01:01:05)：首先，我来介绍一下这个功能。');
    });
    ...
    it('should handle missing speaker_info', () => {
          ...
          paragraphs: [
            {
              ...
              start_time: 60000,
              end_time: 65000,
              speaker_info: {} as SpeakerInfo, // 强制类型转换以模拟缺失的 speaker_info
              sentences: [
                {
                  sid: 's1',
                  start_time: 60000, // 1分钟
                  end_time: 65000,
                  words: [
                    {
                      wid: 'w1',
      ...
      const result = service.formatTranscript(transcript);
      expect(result).toBe('未知发言人(00:01:00)：没有发言人信息');
    });
    ...
    it('should handle missing username in speaker_info', () => {
      ...
      const result = service.formatTranscript(transcript);
      expect(result).toBe('未知发言人(00:02:00)：用户名为空');
    });
