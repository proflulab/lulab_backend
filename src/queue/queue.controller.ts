...
export class QueueController {
    ...
    @ApiBody({
        schema: {
            type: 'object',
            properties: {
                completedMaxAge: { type: 'number', description: 'Max age for completed jobs in milliseconds' },
                failedMaxAge: { type: 'number', description: 'Max age for failed jobs in milliseconds' },
                limit: { type: 'number', description: 'Maximum number of jobs to clean' },
            },
        },
    })
    ...
    @ApiBody({
        schema: {
            type: 'object',
            properties: {
                service: { type: 'string', enum: ['tencent-meeting', 'lark', 'aliyun-sms', 'upload'] },
                action: { type: 'string' },
                payload: { type: 'object' },
                priority: { type: 'number' },
    ...
    async addExternalApiJob(
        ...
        try {
            ...
            switch (body.service) {
                case 'tencent-meeting':
                    job = await this.externalApiQueue.addTencentMeetingSyncJob(
                        { meetingId: body.payload.meetingId, action: body.action as any, meetingData: body.payload },
                        { priority: body.priority },
                    );
                    break;
                case 'lark':
            ...
            return { jobId: job?.id, message: `${body.service} job added successfully` };
        } catch (error) {
            throw new HttpException(
