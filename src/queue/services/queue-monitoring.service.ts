...
export class QueueMonitoringService {
  ...
  private setupQueueMonitoring(queueName: QueueName): void {
    try {
      ...
      this.logger.error(
        `Failed to setup monitoring for queue ${queueName}:`,
        error,
      );
    }
  }
  ...
  private setupEventListeners(
    ...
    queueEvents.on('progress', ({ jobId, data }) => {
      this.logger.debug(
        `Job ${jobId} progress updated in queue ${queueName}:`,
        data,
      );
    });
  ...
  async getQueueHealth(queueName: QueueName): Promise<{
    ...
    if (metrics.waiting > maxWaiting) {
      issues.push(
        `Too many waiting jobs: ${metrics.waiting} (max: ${maxWaiting})`,
      );
    }

    if (metrics.failed > maxFailed) {
      issues.push(
        `Too many failed jobs: ${metrics.failed} (max: ${maxFailed})`,
      );
    }
  ...
  async cleanAllQueues(
    ...
    const results: Partial<
      Record<QueueName, { completed: number; failed: number }>
    > = {};
