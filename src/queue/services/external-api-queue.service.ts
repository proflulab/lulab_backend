...
export class ExternalApiQueueService extends BaseQueueService {
    ...
    async addTencentMeetingSyncJob(
        ...
        const jobData: ExternalApiJobData = {
            idempotencyKey: options?.correlationId || `tencent-${syncData.meetingId}-${syncData.action}-${Date.now()}`,
            userId: options?.userId,
    ...
    async addLarkBitableSyncJob(
        ...
        const jobData: ExternalApiJobData = {
            idempotencyKey: options?.correlationId || `lark-${syncData.appId}-${syncData.tableId}-${syncData.action}-${Date.now()}`,
            userId: options?.userId,
            correlationId: options?.correlationId || randomUUID(),
            createdAt: new Date(),
            service: 'lark',
    ...
    async addWebhookProcessingJob(
        ...
        const jobData: ExternalApiJobData = {
            ...
            correlationId: randomUUID(),
            createdAt: new Date(),
            service: webhookData.source === 'other' ? 'tencent-meeting' : webhookData.source,
        ...
        const jobType = webhookData.source === 'tencent-meeting'
