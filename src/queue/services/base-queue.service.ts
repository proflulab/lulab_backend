...
export abstract class BaseQueueService {
    ...
    async addBulkJobs<T extends BaseJobData>(
        ...
        if (!this.queue) {
            this.logger.warn(`Queue ${this.queueName} not initialized, skipping bulk jobs`);
            return [];
        }
        ...
        try {
            ...
            for (const job of jobs) {
                const isProcessed = await this.checkIdempotency(job.data.idempotencyKey);
                if (!isProcessed) {
                    jobsToAdd.push({
                        name: job.name,
                        data: job.data,
     ...
     */
    async getIdempotencyResult(idempotencyKey: string): Promise<JobResult | null> {
        if (!this.redisService.isReady()) {
            return null;
        }
