...
describe('MeetingQueueService', () => {
    ...
    it('should get correct job type for action', () => {
        const getJobTypeForAction = (service as any).getJobTypeForAction.bind(service);

        expect(getJobTypeForAction('process')).toBe(JobType.PROCESS_MEETING_RECORD);
        expect(getJobTypeForAction('analyze')).toBe(JobType.ANALYZE_MEETING_CONTENT);
    ...
    it('should throw error for unknown action', () => {
        ...
        expect(() => getJobTypeForAction('unknown')).toThrow('Unknown meeting action: unknown');
    });
});