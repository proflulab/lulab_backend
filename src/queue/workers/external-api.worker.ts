...
export class ExternalApiWorker extends BaseWorker<ExternalApiJobData> {
    ...
    protected async processJob(job: Job<ExternalApiJobData>): Promise<JobResult> {
        return this.executeJob(job, async (data) => {
            ...
            this.logger.log(
                `Processing ${service} API job with action ${action}`,
     ...
     */
    private async processTencentMeetingJob(action: string, payload: any): Promise<any> {
        try {
            ...
            switch (action) {
                ...
                default:
                    throw new UnrecoverableError(`Unknown Tencent Meeting action: ${action}`);
            }
        } catch (error) {
     ...
     */
    private async processAliyunSmsJob(action: string, payload: any): Promise<any> {
        try {
    ...
    private async uploadRecording(payload: any): Promise<any> {
        ...
        this.logger.debug(`Uploading recording ${fileName} for meeting ${meetingId} to ${storageProvider}`);
        ...
        this.logger.debug(`Recording uploaded successfully: ${uploadResult.uploadedUrl}`);
        return uploadResult;
    }
    ...
    private async processTencentWebhook(payload: any): Promise<any> {
        ...
        return { eventType, processed: true, processedAt: new Date(), data: webhookPayload };
    }
    ...
    private async updateLarkRecord(payload: any): Promise<any> {
        ...
        this.logger.debug(`Updating Lark record ${recordId} in ${appId}/${tableId}`);
        return { appId, tableId, recordId, status: 'updated', updatedAt: new Date(), data };
    }

    private async deleteLarkRecord(payload: any): Promise<any> {
        const { appId, tableId, recordId } = payload;
        this.logger.debug(`Deleting Lark record ${recordId} in ${appId}/${tableId}`);
        return { appId, tableId, recordId, status: 'deleted', deletedAt: new Date() };
    }

    private async batchCreateLarkRecords(payload: any): Promise<any> {
        const { appId, tableId, records } = payload;
        this.logger.debug(`Batch creating ${records.length} Lark records in ${appId}/${tableId}`);
        return {
            appId,
            tableId,
    ...
    private async batchUpdateLarkRecords(payload: any): Promise<any> {
        const { appId, tableId, records } = payload;
        this.logger.debug(`Batch updating ${records.length} Lark records in ${appId}/${tableId}`);
        return {
            appId,
            tableId,
