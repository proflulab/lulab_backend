...
export class QueueUsageExamples {
    ...
    async processMeetingRecording(meetingId: string, recordingUrl: string) {
        ...
        await this.meetingQueue.addSyncJob(
            meetingId,
            'lark-bitable',
            { recordingUrl },
        );
    ...
    async runAllExamples() {
        ...
        try {
            await this.processMeetingRecording('meeting-001', 'https://example.com/recording.mp4');
            await this.userRegistrationWorkflow('user@example.com', '+1234567890', { name: 'Test User' });
            ...
            await this.bulkMeetingProcessing(['meeting-003', 'meeting-004', 'meeting-005']);
            await this.processWebhookEvent('tencent-meeting', 'meeting.ended', { meetingId: 'tm-123' });
