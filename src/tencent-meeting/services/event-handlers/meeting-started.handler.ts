...
export class MeetingStartedHandler extends BaseEventHandler {
  ...
  async handle(payload: TencentEventPayload, index: number): Promise<void> {
    ...
    try {
      const operatorResult = await this.meetingUserBitable.upsertMeetingUserRecord({
        uuid: operator.uuid,
      ...
      this.logger.log(`成功处理用户信息: ${operator.user_name} (${operator.uuid})`);
    } catch (error) {
    ...
    let creatorRecordId;
    try {
      const creatorResult = await this.meetingUserBitable.upsertMeetingUserRecord({
        uuid: creator.uuid,
      ...
      this.logger.log(`成功处理用户信息: ${creator.user_name} (${creator.uuid})`);
    } catch (error) {
    ...
    try {
      ...
      this.logger.error(`处理会议开始事件失败: ${meeting_info.meeting_id}`, error);
      // 不抛出错误，避免影响主流程
    }
  }
}