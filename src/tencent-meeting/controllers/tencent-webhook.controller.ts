...
import { 
    TencentWebhookEventBodyDto, 
} from '../dto/tencent-webhook-body.dto';
...
export class TencentWebhookController {
    ...
    private getTencentMeetingConfig(): { token: string; encodingAesKey: string } {
        ...
        if (!encodingAesKey) {
            throw new WebhookConfigException('TENCENT_MEETING', 'TENCENT_MEETING_ENCODING_AES_KEY');
        }
    ...
    async handleTencentWebhook(
        @Body(new ValidationPipe({ transform: true, whitelist: true }))body: TencentWebhookEventBodyDto,
        @Headers('timestamp') timestamp: string,
        @Headers('nonce') nonce: string,
        @Headers('signature') signature: string
    ): Promise<string> {
        ...
        try {
            ...
            if (body && body.data) {
                ...
                this.logger.log(`Successfully processed Tencent Meeting event: ${eventData.event}`, {
                ...
                this.tencentEventHandlerService.handleEvent(eventData).catch(error => {
            ...
            throw new BadRequestException('Invalid Webhook request - missing data field');
