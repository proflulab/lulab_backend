...
export class DeepSeekAdapter implements AgentAdapter {
  ...
  async validateConfig(): Promise<boolean> {
    ...
    try {
      ...
      this.logger.log('DeepSeek configuration validated successfully');
      return true;
    } catch (error) {
      this.logger.error('DeepSeek configuration validation failed:', error);
      return false;
    }
  }
  ...
  async complete(
    ...
    if (!this.openai) {
      throw new Error(
        'DeepSeek client not initialized. Check API key configuration.',
      );
    }
    ...
    try {
      ...
      this.logger.error('DeepSeek completion failed:', error);
      throw new Error(
        `DeepSeek completion failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async *completeStream(
    ...
    try {
      ...
      this.logger.error('DeepSeek streaming completion failed:', error);
      throw new Error(
        `DeepSeek streaming completion failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
